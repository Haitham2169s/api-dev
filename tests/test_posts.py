...
@pytest.mark.parametrize("title, content, published", [
    ("awesome new blog", "awesome new content", True),
    ("favourite stretches", "i love stretching", False),
    ("hardest workout", "burpees", True)
])
...
def test_unauthorized_user_delete_posts(client, test_user, test_posts):
    res = client.post(f"/posts/{test_posts[0].id}")
